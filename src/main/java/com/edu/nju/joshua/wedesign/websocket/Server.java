package com.edu.nju.joshua.wedesign.websocket;
...
public class Server {
    ...
    private MessageMapper messageMapper=(MessageMapper) MyApplicationContextAware.getApplicationContext().getBean("messageMapper");
    private UserMapper userMapper=(UserMapper) MyApplicationContextAware.getApplicationContext().getBean("userMapper");
    /**
     * 连接建立成功调用的方法
    ...
    public void onOpen(Session session, EndpointConfig config){
        ...
        this.userId=String.valueOf(((User)httpSession.getAttribute("currentUser")).getId());
        ...
        messageList =messageMapper.getMessageUnReceive(Integer.valueOf(userId));
        if(messageList!=null){
    ...
    public void onMessage(String jsonMessage) throws IOException {
        ...
        message.setFromUserId(jsonObjectMessage.getIntValue("from"));
        message.setContent(jsonObjectMessage.getString("content"));
        message.setMessageType(jsonObjectMessage.getIntValue("type"));
        ...
        if(jsonObjectMessage.getIntValue("type") == 1){
            message.setToUserId(users.getInteger(0));
        ...
        for(int i = 0;i < users.size();i++){
            if(!(jsonObjectMessage.getIntValue("type") == 1 && i==0)) {
                ...
                //分别发送给每个指定用户
                if (!user .equals("null")) {
                    message.setToUserId(Integer.valueOf(user));
...
//			}
    ...
    public String getMessage(Message message){
        ...
        jsonObjectMessage.put("type", String.valueOf(message.getMessageType()));
